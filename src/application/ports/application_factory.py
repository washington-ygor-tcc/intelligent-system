...
from src.core.adapters.mlflow_repository import MLFlowPyFuncRepository
...
class ApplicationFactory(ABC):
    def __init__(self):
        self.__load_config()
        self.__model_gateway = MLFlowPyFuncRepository(
            self._config["mlflow"]["server_uri"],
            self._config["mlflow"]["model_uri"],
        )
        self._prediction_request_handler = PredictUseCase(self.__model_gateway)
